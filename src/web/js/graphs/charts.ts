...
type WordSeriesPoint = { t: number; val: number };
type GraphVocabStatus = "new" | "learning" | "known" | "ignored";
type DonutSlice = { status: GraphVocabStatus; label: string; val: number; color: string; start: number; end: number };

export type ChartRenderer = (entries?: readonly VocabEntry[], options?: ChartOptions) => void;

const t = rawT as (key: string, vars?: TranslationVars) => string;
const stateVocabEntries = (): VocabEntry[] => Object.values(state.vocab || {}) as VocabEntry[];
...
function getCefrThresholds(lang: string): number[] {
  return CEFR_THRESHOLDS[lang] || CEFR_THRESHOLDS.en;
}

function getKnownWordCount(entries: readonly VocabEntry[] = stateVocabEntries()): number {
  return entries.filter(e => e?.status === "known").length;
}

function getKnownLearningWordCount(entries: readonly VocabEntry[] = stateVocabEntries()): number {
  return entries.filter(e => e?.status === "known" || e?.status === "learning").length;
}

function knownAt(entry: VocabEntry): number | null {
  const raw = entry?.knownAt || entry?.updatedAt || entry?.lastReviewedAt || entry?.addedAt;
  const time = raw ? new Date(raw).getTime() : NaN;
  return Number.isFinite(time) ? time : null;
}

function progressAt(entry: VocabEntry): number | null {
...
function buildWordSeries(
  entries: readonly VocabEntry[],
  includeEntry: (entry: VocabEntry) => boolean,
  dateForEntry: (entry: VocabEntry) => number | null,
  _width = 400,
  now = Date.now()
): WordSeriesPoint[] {
...
export function buildKnownWordSeries(entries: readonly VocabEntry[] = stateVocabEntries(), width = 400, now = Date.now()): WordSeriesPoint[] {
  return buildWordSeries(entries, e => e?.status === "known", knownAt, width, now);
}

export function buildKnownLearningWordSeries(entries: readonly VocabEntry[] = stateVocabEntries(), width = 400, now = Date.now()): WordSeriesPoint[] {
  return buildWordSeries(entries, e => e?.status === "known" || e?.status === "learning", progressAt, width, now);
}

export function getCurrentLevel(count: number, thresholds: readonly number[]): string {
  let level = "Pre-A1";
...
export function formatVocabProgressDate(time: number, span: number, locale?: Intl.LocalesArgument): string {
...
function rangeTitle(title: string, allTime: boolean): string {
  return allTime ? `${title.replace(/\s*\([^)]*\)/, "")} · ${t("graphs.rangeAll")}` : title;
}

export function buildAddedOverTimeBins(entries: readonly VocabEntry[] = stateVocabEntries(), allTime = false, now = new Date()): ChartBin[] {
  const dated = entries
...
export function renderDueForecast(_chartEntries?: readonly VocabEntry[], options: ChartOptions = {}): void {
  ...
  const hotAreas: Array<{ x: number; y: number; w: number; h: number; label: string }> = [];
  let bx = pad.left + 1;
  if (overdue > 0) {
  ...
  const canv = document.getElementById("graph-due") as HTMLCanvasElement | null;
  if (canv) {
    canv.onmousemove = (e) => {
...
export function renderStatusDonut(_chartEntries?: readonly VocabEntry[], _options?: ChartOptions): void {
  ...
  for (const e of _chartEntries || stateVocabEntries()) {
    if (e.status === "new" || e.status === "learning" || e.status === "known") counts[e.status]++;
  }
  const total = counts.new + counts.learning + counts.known || 1;
  const slices = ([
    { status: "new", label: t("vocab.statusNew"), val: counts.new, color: C.new, start: 0, end: 0 },
    { status: "learning", label: t("vocab.statusLearning"), val: counts.learning, color: C.learning, start: 0, end: 0 },
    { status: "known", label: t("vocab.statusKnown"), val: counts.known, color: C.known, start: 0, end: 0 }
  ] as DonutSlice[]).filter(s => s.val > 0);
  ...
  const canv = document.getElementById("graph-status") as HTMLCanvasElement | null;
  if (canv) {
...
export function renderIntervalHistogram(_chartEntries?: readonly VocabEntry[], _options?: ChartOptions): void {
  const ctx = canvas("graph-intervals");
  if (!ctx) return;
  const binLabels = t("graphs.binIntervalLabels").split("|");
  const bins: ChartBin[] = binLabels.map(label => ({ label, val: 0 }));
...
export function renderRepetitions(_chartEntries?: readonly VocabEntry[], _options?: ChartOptions): void {
  const ctx = canvas("graph-reps");
  if (!ctx) return;
  const repLabels = t("graphs.binRepsLabels").split("|");
  const bins: ChartBin[] = repLabels.map(label => ({ label, val: 0 }));
...
export function renderAddedOverTime(_chartEntries?: readonly VocabEntry[], options: ChartOptions = {}): void {
  const ctx = canvas("graph-added");
  if (!ctx) return;
  const bins = buildAddedOverTimeBins(_chartEntries || stateVocabEntries(), options.allTime ?? false);
  const maxVal = Math.max(1, ...bins.map(b => b.val));
  ...
  ctx.fillText(rangeTitle(t("graphs.addedOverTime"), options.allTime ?? false), ctx.w / 2, 10);
}

export function renderDayOfWeek(_chartEntries?: readonly VocabEntry[], _options?: ChartOptions): void {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    if (counts[i] > 0) {
      ...
      ctx.fillText(String(counts[i]), x + barW / 2, Math.max(16, pad.top + ph - h - 4));
    }
...
export function renderFsrsScatter(_chartEntries?: readonly VocabEntry[], _options?: ChartOptions): void {
...
export function renderMatureVsYoung(_chartEntries?: readonly VocabEntry[], _options?: ChartOptions): void {
...
export function renderVocabProgress(_chartEntries?: readonly VocabEntry[], _options?: ChartOptions): void {
  ...
  const pw = W - pad.left - pad.right;
  const ph = H - pad.top - pad.bottom;
  const chartEntries = _chartEntries || stateVocabEntries();
  ...
  const xScale = (time: number) => pad.left + ((time - startTime) / (endTime - startTime || 1)) * pw;
  const yScale = (value: number) => Math.log10(Math.max(value, 1)) / logMax;
  const yFor = (value: number) => pad.top + ph - yScale(value) * ph;
  ...
  // Tooltip
  const canv = document.getElementById("graph-vocab-progress") as HTMLCanvasElement | null;
  if (canv) {
    canv.onmousemove = (e) => {
      const r = canv.getBoundingClientRect();
      const mx = e.clientX - r.left;
