...
import type { PdfOcrPage } from "./pdf-page-text.js";
...
let activeCorrectionDialog: HTMLDialogElement | null = null;
let correctionDialogCounter = 0;

export function openPdfOcrCorrection(
  current: unknown,
  pageIndex: number,
  options: CorrectionOptions = {}
): Promise<boolean> {
  if (!current || typeof current !== "object") return Promise.resolve(false);
  const pdfDocument = current as Partial<PdfOcrDocument>;
  const page = pdfDocument.pdfOcrPages?.[pageIndex];
  if (!pdfDocument.id || !page) return Promise.resolve(false);
  if (activeCorrectionDialog?.isConnected) {
    activeCorrectionDialog.querySelector<HTMLTextAreaElement>("textarea")?.focus();
  ...
  const expectedUpdatedAt = pdfDocument.updatedAt || null;
  ...
  const imageUrl = imageName
    ? `/__media?book=${encodeURIComponent(pdfDocument.id)}&img=${encodeURIComponent(imageName)}`
    : "";
  dialog.innerHTML = `
  ...
  return new Promise<boolean>((resolve) => {
    ...
    form.addEventListener("submit", async (event) => {
      ...
      try {
        ...
        const saved = await updatePdfOcrPageText(pdfDocument.id, pageIndex, nextPageText, {
          expectedUpdatedAt
        });
