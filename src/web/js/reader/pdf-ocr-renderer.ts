...
import { buildPdfDocumentText, countEffectivePdfPageWords, effectivePdfPageText, reconcilePdfPageWords } from "./pdf-page-text.js";
...
interface PdfOcrDocument {
  id: string;
  text?: string;
  pdfOcrPages: PdfOcrPage[];
  pdfOcrEngine?: string;
  [key: string]: unknown;
}

interface PdfOcrZoomOptions {
  commit?: boolean;
  focalClientX?: number;
  focalClientY?: number;
}

interface PdfOcrZoomAnchor {
  xRatio: number;
  yRatio: number;
  viewportX: number;
  viewportY: number;
...
interface PdfOcrRect extends Required<Pick<PdfOcrWord, "x" | "y" | "width" | "height">> {}
...
export function isPdfOcrText(text: unknown): text is PdfOcrDocument {
  ...
  const candidate = text as { pdfOcrPages?: unknown };
  return Array.isArray(candidate.pdfOcrPages) && candidate.pdfOcrPages.length > 0;
}
...
export function setPdfOcrViewMode(value: string, options: PdfOcrZoomOptions = {}): "text" | "overlay" {
...
export function setPdfOcrZoom(value: number, options: PdfOcrZoomOptions = {}): number {
  const zoom = normalizePdfOcrZoom(value);
  const container = readerEls.readerText;
  const page = container.querySelector<HTMLElement>(".pdf-ocr-page");
...
export function adjustPdfOcrZoom(delta: number, options: PdfOcrZoomOptions = {}): number {
  return setPdfOcrZoom(getPdfOcrZoom() + delta, options);
}

export function resetPdfOcrZoom(options: PdfOcrZoomOptions = {}): number {
...
export function renderPdfOcrReader(current: PdfOcrDocument, savedPos: unknown): void {
  ...
  const session = getReaderSession({ id: current.id, text: documentText }, language, wordAlgorithm) as ReaderSession;
  ...
  readerEls.readerText.classList.toggle("pdf-text-layer-reader", !overlayMode);

  if (!overlayMode) {
    renderPdfOcrTextMode(current, page, globalOffset, pageCharOffset, totalPages, scrollPerPageKey, savedPos);
    return;
  }
  ...
  const pageWords = reconcilePdfPageWords(
    sourcePageWords,
    effectivePdfPageText(page),
    language,
    wordAlgorithm
  );
  const overlayTokens = pageWords.flatMap<TextToken>((word) => {
    const raw = String(word?.text || "");
    const trailing = raw.match(/[^\p{L}\p{N}'’]+$/u)?.[0] || "";
    return [
      { type: "word", value: trailing ? raw.slice(0, -trailing.length) : raw },
      { type: "text", value: trailing || " " }
    ];
  });
  const classifications = classifyTokenOccurrences(overlayTokens, state.vocab, language);
  const overlayWordPositions = mapPdfOverlayWordPositions(
    pageWords,
    effectivePdfPageText(page),
    language,
    wordAlgorithm,
  ...
  const wordsHtml = pageWords
    .map((word, index) => {
      const position = overlayWordPositions[index];
      const globalIndex = position?.wordIndex ?? null;
  ...
  readerEls.readerText.innerHTML = `
...
function renderPdfOcrTextMode(
  current: PdfOcrDocument,
  page: PdfOcrPage,
  globalOffset: number,
  pageCharOffset: number,
  totalPages: number,
  scrollPerPageKey: string,
  savedPos: unknown
): void {
  ...
  const textHtml = renderPdfOcrTextTokens(
    tokens,
    globalOffset,
    pageCharOffset,
    classifyTokenOccurrences(tokens, state.vocab, language)
  );
...
function renderPdfOcrTextTokens(
  tokens: readonly TextToken[],
  globalOffset: number,
  pageCharOffset: number,
  classifications: Map<number, { key: string; status: string }>
): string {
  let html = "";
  let wordCount = 0;
  let charOffset = pageCharOffset;
  for (let tokenIndex = 0; tokenIndex < tokens.length; tokenIndex++) {
    const token = tokens[tokenIndex];
    if (token.type !== "word") {
      html += escapeHtml(token.value || "");
      charOffset += token.type === "image" ? `[IMG:${token.value}]`.length : token.value.length;
      continue;
    }
...
function normalizePdfOcrZoom(value: number): number {
  const zoom = Number(value);
  return clamp(Number.isFinite(zoom) ? zoom : 1, PDF_OCR_ZOOM_MIN, PDF_OCR_ZOOM_MAX);
}

function pdfOcrZoomLayout(zoom: number) {
...
function renderPdfOcrToolbar(zoom: number, viewMode: string): string {
...
function applyPdfOcrZoomToDom(zoom: number): void {
  const container = readerEls.readerText;
  const stage = container.querySelector<HTMLElement>(".pdf-ocr-stage");
  const page = container.querySelector<HTMLElement>(".pdf-ocr-page");
  ...
  const value = container.querySelector<HTMLElement>("[data-pdf-zoom-value]");
  if (value) value.textContent = `${Math.round(zoom * 100)}%`;
  const zoomOut = container.querySelector<HTMLButtonElement>("[data-pdf-zoom='out']");
  if (zoomOut) zoomOut.disabled = zoom <= PDF_OCR_ZOOM_MIN + 0.001;
  const zoomIn = container.querySelector<HTMLButtonElement>("[data-pdf-zoom='in']");
  if (zoomIn) zoomIn.disabled = zoom >= PDF_OCR_ZOOM_MAX - 0.001;
}

function getPdfOcrZoomAnchor(container: HTMLElement, page: HTMLElement, options: PdfOcrZoomOptions): PdfOcrZoomAnchor | null {
...
function restorePdfOcrZoomAnchor(container: HTMLElement, page: HTMLElement, anchor: PdfOcrZoomAnchor): void {
...
function getPdfOcrPageWords(page: PdfOcrPage): PdfOcrWord[] {
...
export function mapPdfOverlayWordIndexes(
  pageWords: readonly PdfOcrWord[],
  pageText: string,
  lang: string,
  algorithm: string,
  globalOffset = 0
): Array<number | null> {
  return mapPdfOverlayWordPositions(pageWords, pageText, lang, algorithm, globalOffset)
    .map((position) => position?.wordIndex ?? null);
}

interface PdfOverlayWordPosition {
  wordIndex: number;
  charOffset: number;
}

export function mapPdfOverlayWordPositions(
  pageWords: readonly PdfOcrWord[],
  pageText: string,
  lang: string,
  algorithm: string,
  globalOffset = 0,
  pageCharOffset = 0
): Array<PdfOverlayWordPosition | null> {
  const normalizeOverlayWord = (value: unknown) => normalizeWord(value)
    .replace(/^[^\p{L}\p{N}'’]+|[^\p{L}\p{N}'’]+$/gu, "");
  const overlayWords = (pageWords || []).map((word) => normalizeOverlayWord(word?.text || ""));
  let charOffset = pageCharOffset;
  const correctedWords = tokenizeText(pageText, lang, algorithm).flatMap((token) => {
    const start = charOffset;
    charOffset += token.type === "image" ? `[IMG:${token.value}]`.length : token.value.length;
    return token.type === "word" ? [{ value: normalizeOverlayWord(token.value), charOffset: start }] : [];
  });
  const result: Array<PdfOverlayWordPosition | null> = new Array(overlayWords.length).fill(null);
  if (overlayWords.length === correctedWords.length
    && overlayWords.every((word, index) => word && word === correctedWords[index].value)) {
    return correctedWords.map((word, index) => ({
      wordIndex: globalOffset + index,
  ...
  for (let left = overlayWords.length - 1; left >= 0; left -= 1) {
    for (let right = correctedWords.length - 1; right >= 0; right -= 1) {
      ...
      lengths[index] = overlayWords[left] && overlayWords[left] === correctedWords[right].value
  ...
  while (left < overlayWords.length && right < correctedWords.length) {
    if (overlayWords[left] && overlayWords[left] === correctedWords[right].value) {
      result[left] = {
        wordIndex: globalOffset + right,
        charOffset: correctedWords[right].charOffset
      };
      left += 1;
      right += 1;
...
function layoutPdfOcrLineWords(line: PdfOcrLine, page: PdfOcrPage): PdfOcrWord[] {
...
function splitPdfOcrTextRuns(value: string): PdfOcrRun[] {
  const text = String(value || "").trim();
  const runs: PdfOcrRun[] = [];
  const pattern = /\S+/gu;
...
function measurePdfOcrRuns(text: string, runs: readonly PdfOcrRun[], lineHeight: number): PdfOcrRunSpan[] | null {
...
function getPdfOcrMeasureContext(): CanvasRenderingContext2D | null {
...
function estimatePdfOcrRuns(text: string, runs: readonly PdfOcrRun[]): PdfOcrRunSpan[] {
...
function pdfOcrRunRatio(run: PdfOcrRun, start: number, width: number, total: number): PdfOcrRunSpan {
  return {
    text: run.text,
...
function pdfOcrTextWeight(text: string): number {
  let total = 0;
  for (const ch of String(text || "")) total += pdfOcrGlyphWeight(ch);
  return total;
}

function pdfOcrGlyphWeight(ch: string): number {
...
function pdfOcrRect(item: PdfOcrWord, page: PdfOcrPage, current?: PdfOcrDocument) {
...
function correctLegacyPdfTextLayerRect(rect: PdfOcrRect, page: PdfOcrPage, current?: PdfOcrDocument): PdfOcrRect {
...
function usesLegacyPdfTextLayerBounds(page: PdfOcrPage, current?: PdfOcrDocument): boolean {
...
function renderPdfOcrWord(
  item: PdfOcrWord,
  page: PdfOcrPage,
  globalIndex: number | null,
  current: PdfOcrDocument,
  matchedWord = "",
  pageWordIndex: number | null = null,
