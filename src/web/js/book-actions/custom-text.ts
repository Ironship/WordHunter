...
import type { PdfOcrPage } from "../reader/pdf-page-text.js";
...
export interface CustomTextMeta {
  id?: string;
  author?: string;
  source?: string;
  level?: string;
...
interface UpdatePdfOcrOptions {
  expectedUpdatedAt?: string | null;
}

interface PdfCustomText extends WhText {
  pdfOcrPages?: PdfOcrPage[];
  pdfOcrEngine?: string;
  pdfOcrPageCount?: number;
  experimental?: boolean;
...
export async function importCustomText(
  title: string,
  text: string,
  meta: CustomTextMeta = {},
  ...
  const customText: PdfCustomText = {
    id: id,
    title: cleanTitle,
...
export async function updatePdfOcrPageText(
  id: string,
  pageIndex: number,
  correctedText: string,
  options: UpdatePdfOcrOptions = {}
): Promise<boolean> {
  const existing = findCustomText(id, { coerce: true }) as PdfCustomText | undefined;
