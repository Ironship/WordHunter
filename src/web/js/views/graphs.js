...
function graphSignature(containers) {
  return containers.map((c) => `${c.id}:${c.wide ? "wide" : "normal"}`).join("|");
}

function ensureGraphCanvases(el, containers) {
...
function revealGraphCanvas(id, index) {
...
export function renderGraphs() {
  ...
  if (!Object.keys(state.vocab).length) {
    delete el.dataset.graphSignature;
    delete el.dataset.graphRendered;
  ...
  ensureGraphCanvases(el, GRAPH_CONTAINERS);
  setGraphsLoading(el.dataset.graphRendered !== "1");
