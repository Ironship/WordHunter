...
function elapsedDaysSince(dateISO, now) {
  if (!dateISO) return 0;
...
export function calculateFSRS(quality, prev, now = new Date()) {
  ...
  let nextStability;
  if (isFirstReview) {
    nextStability = { again: 1, hard: 2, good: 4, easy: 7 }[rating];
  ...
  const difficultyDelta = { again: 1.15, hard: 0.45, good: -0.15, easy: -0.65 }[rating];
...
export function ensureSM2Fields(entry) {
