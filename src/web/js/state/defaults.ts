...
import { DEFAULT_SELECTED_WORD_PANEL_ITEMS, STATE_SCHEMA_VERSION, UI_SCALE } from "../constants.js";
import { VOCAB_STATUS_FILTERS } from "../events/vocab-status.js";
import { DEFAULT_LEARNING_COLORS } from "../reader-colors.js";
import { DEFAULT_THEME } from "../theme.js";

export function normalizeVocabStatusFilters(value: unknown): WhVocabStatus[] {
  const statuses = VOCAB_STATUS_FILTERS as WhVocabStatus[];
  if (Array.isArray(value)) return value.filter((status): status is WhVocabStatus => statuses.includes(status));
  return [...statuses];
}

export function normalizeAnkiExportStatuses(value: unknown): WhVocabStatus[] {
  const allowedStatuses = VOCAB_STATUS_FILTERS as WhVocabStatus[];
  if (!Array.isArray(value)) return ["learning"];
  const statuses = value.filter((status): status is WhVocabStatus => allowedStatuses.includes(status));
  return statuses.length ? statuses : ["learning"];
}

export function getDefaultDictionaryUrl(lang: string): string {
...
export function createDefaultState(): WhAppState {
  ...
  return {
    ...
    preferences: {
      ...
      selectedWordPanelItems: DEFAULT_SELECTED_WORD_PANEL_ITEMS.map((item) => ({ ...item })),
      touchControls: false,
