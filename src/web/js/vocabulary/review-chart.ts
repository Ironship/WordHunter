...
interface ReviewEntry extends VocabEntry {
  word: string;
  nextDate: string;
}
...
export function renderReviewChart(srsEntries: readonly ReviewEntry[], today: string): void {
  if (!reviewEls.reviewChart) return;
  ...
  if (graphType === "heatmap") {
    ...
    requestAnimationFrame(() => {
      ...
      if (!context) return;
      const ctx = context as ChartContext;
      ctx.scale(dpr, dpr);
      ctx.w = W; ctx.h = H;
      ...
      if (graphType === "dueForecast") {
        ...
        if (overdue > 0) {
          ...
          ctx.fillText(String(overdue), bx + barW / 2, pad.top + ph - h - 2);
          bx += barW + 3;
        }
        for (let d = 0; d < days; d++) {
          ...
          if (buckets[d] > 0 && barW > 10) {
            ctx.fillStyle = ink; ctx.font = "bold 9px Inter, sans-serif"; ctx.textAlign = "center"; ctx.textBaseline = "bottom";
            ctx.fillText(String(buckets[d]), bx + d * (barW + 3) + barW / 2, pad.top + ph - h - 2);
          }
        }
...
export function renderReviewUpcoming(srsEntries: readonly ReviewEntry[], today: string): void {
  if (!reviewEls.reviewUpcoming) return;
  ...
  reviewEls.reviewUpcoming.innerHTML = `<ul class="upcoming-list">${rows}</ul>${more}`;
}
