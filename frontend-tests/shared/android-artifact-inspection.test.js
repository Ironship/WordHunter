...
describe("Android release artifact assertions", () => {
  ...
  it("pins the tauri.conf.json contract the APK/AAB assertions rely on", () => {
    ...
    assert.equal(tauriConfig.version, "1.0.13-rc.5");
    assert.equal(androidVersionCodeFor(tauriConfig.version), 101001305);
  ...
  it("parses the text manifest from the AAB archive root", () => {
    const text = '<manifest xmlns:android="http://schemas.android.com/apk/res/android" package="com.wordhunter.pocket" versionCode="101001101" versionName="1.0.13-rc.5">';
    assert.deepEqual(parseTextManifest(text), {
      versionCode: 101001101,
      versionName: "1.0.13-rc.5",
      debuggable: false,
    });
    assert.equal(
  ...
  it("parses the binary AXML manifest from the AAB archive root", () => {
    // Minimal synthetic AXML: header + string pool ("1.0.13-rc.5") + resource
    // map + one start-element carrying versionCode/versionName/debuggable by
    // resource id.
    const versionName = "1.0.13-rc.5";
    ...
    assert.equal(parsed.versionName, "1.0.13-rc.5");
  ...
  it("parses the protobuf manifest from the AAB base/ directory", () => {
    ...
    const element = Buffer.concat([
      ...
      ld(5, attribute(0x0101021c, typed(0x10, 0, "1.0.13-rc.5"))),
    ...
    assert.equal(bare.versionName, "1.0.13-rc.5");
    ...
    const namedElement = Buffer.concat([
      ...
      ld(5, namedAttribute("versionName", typed(0x10, 0, "1.0.13-rc.5"))),
    ...
    assert.equal(named.versionName, "1.0.13-rc.5");
