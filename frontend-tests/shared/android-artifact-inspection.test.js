...
describe("Android release artifact assertions", () => {
  it("derives the Android versionCode the way tauri-cli 2.11.4 does", () => {
    ...
    assert.equal(androidVersionCodeFor("1.1.0"), 101100099);
    assert.equal(androidVersionCodeFor("2.3.4-rc.5"), 201300405);
  ...
  it("pins the tauri.conf.json contract the APK/AAB assertions rely on", () => {
    ...
    // The Android overlay pins the code the Tauri CLI would derive from the
    // app version, so the APK/AAB assertions expect that same value.
    assert.equal(androidConfig.bundle.android.versionCode, androidVersionCodeFor(tauriConfig.version));
  ...
  it("parses the text manifest from the AAB archive root", () => {
    const text = '<manifest xmlns:android="http://schemas.android.com/apk/res/android" package="com.wordhunter.pocket" versionCode="101001101" versionName="1.0.13">';
  ...
  it("parses the binary AXML manifest from the AAB archive root", () => {
    // Minimal synthetic AXML: header + string pool ("1.0.13") + resource
    ...
    assert.equal(parsed.versionName, "1.0.13");
  ...
  it("parses the protobuf manifest from the AAB base/ directory", () => {
    ...
    const element = Buffer.concat([
      ...
      ld(5, attribute(0x0101021c, typed(0x10, 0, "1.0.13"))),
    ...
    assert.equal(bare.versionName, "1.0.13");
    ...
    const namedElement = Buffer.concat([
      ...
      ld(5, namedAttribute("versionName", typed(0x10, 0, "1.0.13"))),
    ...
    assert.equal(named.versionName, "1.0.13");
