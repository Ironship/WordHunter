...
fn extract_overlay_pages(
    ...
    let mut document = pdf_extract::Document::load_mem(data)
        .map_err(|e| format!("Could not read the PDF text layer: {e}"))?;
    if document.is_encrypted() {
        document
            .decrypt("")
            .map_err(|e| format!("Could not decrypt the PDF text layer: {e}"))?;
    }
...
mod tests {
    use super::{
        Bounds, OverlayWord, merge_words_using_plain_text, split_words_using_plain_text,
