...
static ENCODING_DECLARATION: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"(?i)(?:encoding\s*=\s*|charset\s*=\s*)[\"']?([a-z0-9._-]+)"#)
        .expect("valid encoding declaration regex")
});
static TRAILING_SPACE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"[ \t]+\n").expect("valid trailing space regex"));
static MULTI_NEWLINE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"\n{3,}").expect("valid newline regex"));
static XHTML_SKIP_TAGS: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?is)<(?:script|style|head|svg|math)\b[^>]*>.*?</(?:script|style|head|svg|math)>")
        .expect("valid skipped tag regex")
});
static XHTML_BLOCK_TAGS: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?i)</?(p|div|section|article|chapter|br|li|tr|h[1-6])\b[^>]*>")
        .expect("valid block tag regex")
});
static XHTML_TAGS: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"(?s)<[^>]+>").expect("valid tag regex"));
static XHTML_WHITESPACE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"\s+").expect("valid whitespace regex"));
static XHTML_BOUNDARIES: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"(?:\s*\u{E000}\s*)+").expect("valid repeated boundary regex"));
...
pub(crate) fn decode_epub_text(data: &[u8]) -> String {
    ...
    let declared = ENCODING_DECLARATION
        .captures(&header)
